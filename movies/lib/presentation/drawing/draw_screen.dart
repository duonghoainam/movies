...
import 'package:movies/presentation/drawing/app_image_pick_field.dart';

class DrawScreen extends StatefulWidget {
  static const String routeName = 'draw';
...
class _DrawScreenState extends State<DrawScreen> {
        ...
        children: <Widget>[
          AppImagePickField(
            margin: const EdgeInsets.only(
                right: 15, left: 10, top: 10),
            initialValue: null,
            // state.type == FormType.add ? null : state.image,
            // onChange: notifier.updateImage,
          ),
          // AspectRatio(
          //   aspectRatio: 2 / 1,
          //   child: Row(
  ...
  Future<void> addDrawParts(List<DrawPart> parts) async {
    ...
    final tu = TimeUtils();
    if (bytes == null) {
      bytes = await loadFromAsset(AppString.assetUrlLogo);
    }
